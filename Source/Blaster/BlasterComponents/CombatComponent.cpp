...
void UCombatComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (Character && Character->IsLocallyControlled())
	{
		...
		FVector LocationAboveImpact = HitResult.ImpactPoint + 1.f * HitResult.ImpactNormal.GetSafeNormal();
...
/*
function tied to "Reload" animation montage on the "shell" notifies, note blueprint callable in the .h
in BlasterAnim_BP event graph we call it
by including blasteraniminstance.h we can use montage_jump to section in UpdateShotgunAmmoValues()
*/
